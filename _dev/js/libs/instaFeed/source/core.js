;(function($) {
"use strict";
// instaFeed By Dhananjaya @dkarandana 2016-10-14
				...
				if( $element.length > 0 ){
					...
					if( typeof opts.wrap == "number" && opts.wrap > 0){
						...
						for(var i = 0; i < wrapper.length; i+= opts.wrap ) {
							wrapperTmpString = opts.API.template( opts.wrappertemplate , {
                                wrapcount:wrapcount++
                            }, opts );

					 	 wrapper.slice(i, i + opts.wrap ).wrapAll( wrapperTmpString );// "<div class='wrap wrap_"+ (i+1) + "'></div>");
						}
					}
				}
			},
